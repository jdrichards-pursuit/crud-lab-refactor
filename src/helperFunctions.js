...
export function formSubmitHandle(
  ...
  const fetchData = !paramVar
    ? fetchFunction(endpoint, obj)
    : fetchFunction(endpoint, obj, paramVar);

  fetchData
    .then((resp) => navigateVar(`/screen/${endpoint}/${resp.id}`))
...
export function convertEndpoint(
  ...
  singleUpperCase = false,
  singleLowerCase = false
) {
  if (firstUpperCase) {
  ...
  if (singleUpperCase) {
    const upperPlural = `${string.slice(0, 1).toUpperCase()}${string
      .slice(1)
      .toLowerCase()}`;
